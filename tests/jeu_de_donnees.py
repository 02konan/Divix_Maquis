"""Un maquis complet et vraisemblable, pour les tests.

Ce jeu de données ne part pas en production : `donnees_demo.py` n'installe que
les données dont le logiciel a besoin pour fonctionner. Ici on peuple au
contraire un établissement entier — carte, salle, personnel, trois semaines de
commandes et d'encaissements — parce que la plupart des tests ont besoin de
données à lire, et de données réalistes plutôt que de deux lignes bricolées.
"""
...
from backend import etablissement, modules  # noqa: E402
from backend.auth import creer_utilisateur  # noqa: E402
from backend.database import valeur, initialiser_base, connexion  # noqa: E402
from backend.roles import ROLE_PLATEFORME  # noqa: E402
...
# Les catégories ne sont pas listées ici : chaque établissement reçoit les
# siennes à sa création (`etablissement.CATEGORIES_PAR_DEFAUT`). Les articles
# ci-dessous s'y rattachent par leur nom.
